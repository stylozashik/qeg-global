...
class PaymentController extends Controller
{
    public function index()
    {
        ...
        $items = Payment::all();
        return view('backend.default.Payment.index', compact('items'));
    }
    ...
    public function create()
    {
        ...
        return view('backend.default.Payment.create');
    }
    ...
    {
        $item = Payment::where('code', $code)->first();
        if(!$item)
            return back()->withErrors('Information not found.');

        View::share('title', 'Single item information.');
        return view('backend.default.Payment.show', compact('item'));
    }
    ...
    public function edit($code)
    {
        ...
        return view('backend.default.Payment.edit', compact('item'));
    }


    public function update(UpdatePaymentRequest $request, $code)
    {
        $item = Payment::where('code', $code)->first();
        if(!$item)
            return back()->withErrors('Information not found.');
    ...
    public function destroy($code)
    {
         $item = Payment::where('code', $code)->first();
         if (!$item) {
             $item = Payment::withTrashed()->where('code', $code)->first();
             if (!$item)
                 return back()->withErrors('Information not found.');
